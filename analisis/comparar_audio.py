...
def f5_envelope(samples: np.ndarray, sample_rate: int) -> tuple[np.ndarray, np.ndarray]:
    divisor = sample_rate // 1_000
    reduced = resample_poly(samples, 1, divisor)
    reduced_rate = sample_rate / divisor
...
def make_spectrum_figure(pds: dict[int, tuple[int, np.ndarray]], jpl: tuple[int, np.ndarray], output: Path) -> None:
    ...
    selection = (frequency >= 60) & (frequency <= 250)
    ax.plot(frequency[selection], power[selection], color="#d97706", linewidth=2.0, label="Ensayo JPL · 2.277 rpm")
    ...
    for flight, (sample_rate, samples) in pds.items():
        ...
        selection = (frequency >= 60) & (frequency <= 250)
        ax.plot(frequency[selection], power[selection], linewidth=1.35, alpha=0.9, color=colors[flight], label=f"Perseverance · vuelo {flight}")
    for value, label in [(75.9, "75,9 Hz"), (84.4, "84,4 Hz"), (151.8, "151,8 Hz"), (168.8, "168,8 Hz")]:
        ...
        ax.text(value + 1.0, -4, label, rotation=90, va="top", ha="left", fontsize=8, color="#475569")
    ax.set_xlim(60, 250)
    ax.set_ylim(-45, 2)
    ax.set_title("Firma tonal normalizada: ensayo terrestre frente a cuatro vuelos en Marte", loc="left", weight="bold")
    ax.set_xlabel("Frecuencia (Hz)")
    ax.set_ylabel("Densidad espectral relativa (dB; máximo de cada registro = 0)")
    ax.grid(True, color="#cbd5e1", linewidth=0.6, alpha=0.55)
    ax.legend(ncol=2, frameon=False, loc="lower left")
    fig.savefig(output / "audio-espectros-jpl-perseverance.png", dpi=180)
    plt.close(fig)


def make_null_figure(flight5: tuple[int, np.ndarray], jpl: tuple[int, np.ndarray], output: Path) -> None:
    ...
    axes[0].scatter(JPL_DIPS, np.interp(JPL_DIPS, jpl_time, jpl_comb), color="#991b1b", s=20, zorder=3, label="mínimos medidos")
    ...
    axes[0].set_title("Ensayo JPL: energía conjunta de los armónicos 1–12", loc="left", weight="bold")
    axes[0].set_ylabel("Nivel relativo (dB)")
    axes[0].legend(frameon=False, loc="lower left")

    axes[1].plot(f5_time, f5_env, color="#0f766e", linewidth=1.3)
    axes[1].scatter(F5_NULLS, np.interp(F5_NULLS, f5_time, f5_env), color="#991b1b", s=24, zorder=3, label="nulos medidos")
    ...
    axes[1].set_title("Vuelo 5: envolvente de 81,5–87 Hz; líneas discontinuas = tiempos publicados", loc="left", weight="bold")
    axes[1].set_xlabel("Tiempo del registro (s)")
    axes[1].set_ylabel("Nivel relativo (dB)")
    axes[1].legend(frameon=False, loc="lower left")
    for ax in axes:
        ax.grid(True, color="#cbd5e1", linewidth=0.6, alpha=0.55)
    fig.savefig(output / "audio-nulos-jpl-vuelo5.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    make_spectrum_figure(pds, jpl, args.output)
    make_null_figure(pds[5], jpl, args.output)
